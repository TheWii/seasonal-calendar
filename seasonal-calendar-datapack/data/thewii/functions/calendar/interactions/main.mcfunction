...
execute if score $season twsc.data matches 2 as @a[tag=!global.ignore,nbt=!{ActiveEffects:[{Id:30b}]}] at @s if predicate thewii:calendar/is_swimming if predicate thewii:calendar/in_overworld if predicate thewii:calendar/chances/summer_swim_boost run function calendar:interactions/success/summer_swim_boost
